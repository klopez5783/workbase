  ...
  const handleCopyCode = async () => {
    if (company?.joinCode) {
      try {
        ...
        alert(t('adminTools.copyCode.failedToCopy'));
      }
    }
  };

  const handleRegenerateCode = async () => {
    if (!window.confirm(t('adminTools.regenerateCode.confirmMessage'))) {
      return;
    }
    ...
    try {
      ...
      alert(t('adminTools.regenerateCode.success'));
    } catch (err) {
      console.error('Error regenerating code:', err);
      alert(t('adminTools.regenerateCode.error') + err.message);
    } finally {
    ...
    return (
      <div className="bg-yellow-50 border-l-4 border-yellow-500 rounded-lg p-4">
        <p className="text-yellow-800 text-sm font-medium">
          {t('adminTools.noJoinCode.message')}
        </p>
        <button
        ...
        >
          {regenerating ? t('adminTools.noJoinCode.generating') : t('adminTools.noJoinCode.generateButton')}
        </button>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <div className="flex items-start justify-between mb-4">
        <div>
          <h3 className="text-lg font-bold text-gray-900">{t('adminTools.joinCode.title')}</h3>
          <p className="text-sm text-gray-600 mt-1">
            {t('adminTools.joinCode.description')}
          </p>
        </div>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg p-6 mb-4">
        <div className="text-center">
          <p className="text-sm text-gray-600 mb-2">{t('adminTools.joinCode.label')}</p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        >
          {copied ? (
            <>
              <CheckCircle size={18} />
              {t('adminTools.buttons.copied')}
            </>
          ) : (
            <>
              <Copy size={18} />
              {t('adminTools.buttons.copyCode')}
            </>
          )}
        </button>
        ...
        >
          <Send size={18} />
          {t('adminTools.buttons.sendInvite')}
        </button>
        ...
        >
          {regenerating ? (
            <>
              ...
              {t('adminTools.buttons.regenerating')}
            </>
          ) : (
            <>
              <RefreshCw size={18} />
              {t('adminTools.buttons.regenerate')}
            </>
          )}
        </button>
      </div>
      ...
      <div className="mt-6 bg-gray-50 rounded-lg p-4">
        <h4 className="text-sm font-semibold text-gray-900 mb-2">
          {t('adminTools.howToShare.title')}
        </h4>
        <ul className="text-sm text-gray-700 space-y-2">
          <li className="flex items-start gap-2">
            <span className="text-blue-600 font-bold">1.</span>
            <span>{t('adminTools.howToShare.step1')}</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="text-blue-600 font-bold">2.</span>
            <span>{t('adminTools.howToShare.step2')}</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="text-blue-600 font-bold">3.</span>
            <span>{t('adminTools.howToShare.step3')}</span>
          </li>
          <li className="flex items-start gap-2">
            <span className="text-blue-600 font-bold">4.</span>
            <span>{t('adminTools.howToShare.step4')}</span>
          </li>
        </ul>
      </div>
...
function SendInviteModal({ company, onClose }) {
  ...
  const handleSendInvite = async (e) => {
    ...
    if (digits.length !== 10) {
      setError(t('adminTools.inviteModal.invalidPhone'));
      return;
    }
    ...
    try {
      ...
      setError(err.message || t('adminTools.inviteModal.sendError'));
    } finally {
      setSending(false);
    }
  };
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full">
        ...
        <div className="bg-gradient-to-r from-green-600 to-green-700 px-6 py-4 flex items-center justify-between rounded-t-2xl">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-white">{t('adminTools.inviteModal.title')}</h2>
          </div>
          <button
            onClick={onClose}
          ...
          {success ? (
            <div className="text-center py-8">
              ...
              <h3 className="text-2xl font-bold text-gray-800 mb-2">
                {t('adminTools.inviteModal.successTitle')}
              </h3>
              <p className="text-gray-600">
                {t('adminTools.inviteModal.successMessage')}
              </p>
            </div>
          ) : (
            <form onSubmit={handleSendInvite} className="space-y-6">
              ...
              <div className="bg-blue-50 border-l-4 border-blue-500 rounded-lg p-4">
                <p className="text-blue-900 font-medium text-sm mb-1">
                  {t('adminTools.inviteModal.companyLabel')}: {company.name}
                </p>
                <p className="text-blue-700 text-sm">
                  {t('adminTools.inviteModal.joinCodeLabel')}: <span className="font-mono font-bold">{company.joinCode}</span>
                </p>
              </div>

              <div>
                <label className="block text-sm font-semibold text-gray-700 mb-2">
                  {t('adminTools.inviteModal.phoneLabel')}
                </label>
                <input
                  type="tel"
                  value={phoneNumber}
                ...
                <p className="text-xs text-gray-500 mt-2">
                  {t('adminTools.inviteModal.phoneHint')}
                </p>
              </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  {sending ? t('adminTools.inviteModal.sending') : t('adminTools.inviteModal.sendButton')}
                </button>
              </div>
            </form>
          )}
